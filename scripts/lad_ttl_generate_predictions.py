...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LAD diffusion inference with a trained TTL LoRA adapter and export generated_predictions.jsonl."
    )
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to LAD .pth checkpoint.")
    parser.add_argument("--ttl_adapter_dir", type=str, required=True, help="Path to trained TTL LoRA adapter dir.")
    parser.add_argument(
        "--base_model_name",
        type=str,
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="saves/lad-ttl/predict/generated_predictions.jsonl",
    ...
    parser.add_argument(
        "--merge_lad_lora",
        action="store_true",
        help="Merge LAD LoRA into backbone before loading TTL adapter. Enable if TTL was trained on merged backbone.",
    )
...
def load_ttl_adapter(model: torch.nn.Module, adapter_dir: str) -> None:
    from peft import PeftModel

    adapter_path = Path(adapter_dir)
    if not (adapter_path / "adapter_config.json").exists():
        raise FileNotFoundError(f"TTL adapter config not found: {adapter_path / 'adapter_config.json'}")
    ...
    if hasattr(model.llama, "set_adapter"):
        model.llama.set_adapter("default")
    model.eval()


def maybe_merge_lad_lora(model: torch.nn.Module, merge_lad_lora: bool) -> None:
    if not merge_lad_lora:
        return
    if hasattr(model, "llama") and hasattr(model.llama, "merge_and_unload"):
        model.llama = model.llama.merge_and_unload()
        print("[info] Merged LAD LoRA into backbone before loading TTL adapter.")
...
def main() -> None:
    ...
    maybe_merge_lad_lora(model, args.merge_lad_lora)
    load_ttl_adapter(model, args.ttl_adapter_dir)
    ...
    progress_desc = "LAD+TTL inference" if world_size == 1 else f"LAD+TTL inference (rank {rank}/{world_size})"
